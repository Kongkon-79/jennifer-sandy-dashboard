...
import { LandlordSubmission, LandlordSubmissionsApiResponse } from "./landlord-contact-data-type";
import LandlordContactView from "./landlord-contact-view";

const LandlordContactContainer = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [selectViewContact, setSelectViewContact] = useState(false);
  const session = useSession();
  const token = (session?.data?.user as { accessToken: string })?.accessToken;
  const [selectedContact, setSelectedContact] = useState<LandlordSubmission | null>(null);
  const [selectedContactId, setSelectedContactId] = useState("");
  const queryClient = useQueryClient();

  const { data, isLoading, error, isError } = useQuery<LandlordSubmissionsApiResponse>({
    queryKey: ["landlord-management", currentPage],
    queryFn: async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/landlord?page=${currentPage}&limit=8`,
        {
          method: "GET",
          headers: {
  ...
  } else if (data && landlords.length === 0) {
    content = (
      <div>
          ...
          {landlords.map((item, index) => {
            return (
              <TableRow key={index} className="">
                ...
                <TableCell className="text-base font-normal text-[#68706A] leading-[150%] text-center py-4">
                  {item?.propertyAddress ? item.propertyAddress : "N/A"}
                </TableCell>
                <TableCell className="text-base font-normal text-[#68706A] leading-[150%] text-center py-4">
                  {item?.propertyType ? item.propertyType : "N/A"}
                </TableCell>
                ...
                <TableCell className="w-[400px] text-base font-normal text-[#68706A] leading-[150%] text-center py-4">
                  {item?.status === "new" ? (
                    ...
                    <span className="px-8 py-2 text-[#2FBF71] text-base font-medium leading-[150%] bg-[#EAF9F1]">
                      Contacted
                    </span>
                  )}
                </TableCell>
  ...
  const { mutate } = useMutation({
    mutationKey: ["delete-landlord"],
    mutationFn: async (id: string) => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/landlord/${id}`,
        {
          method: "DELETE",
          headers: {
    ...
    onSuccess: (data) => {
      ...
      toast.success(data?.message || "Landlord deleted successfully");
      queryClient.invalidateQueries({ queryKey: ["landlord-management"] });
    },
  });
      ...
      <div className="p-6 space-y-6">
        ...
        {deleteModalOpen && (
          <DeleteModal
            ...
            title="Are You Sure?"
            desc="Are you sure you want to delete this Landlord?"
          />
        )}
...
export default LandlordContactContainer;
